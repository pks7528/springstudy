...
public class BoardControllerTest {
	...
	*/
	...
	@Autowired
	private WebApplicationContext webAllpicationContext;
	...
	public void setUp() throws Exception {
		mockMvc = MockMvcBuilders.webAppContextSetup(webAllpicationContext).build();
	}
	...
	public void a3상세조회테스트() throws Exception {
		LOGGER.debug(mockMvc.perform(MockMvcRequestBuilders
				.get("/board/detail.do")		// @GetMapping("/boadr/detail.do")
					.param("boardNo", "1"))		// 파라미터
						.andReturn()			// 상세조회결과
	...
	public void a4목록테스트() throws Exception {
		LOGGER.debug(mockMvc.perform(MockMvcRequestBuilders
				.get("/board/list.do"))		// @GetMapping("/boadr/list.do")
					.andReturn()			// 목록조회결과
					.getModelAndView()		// Model에 저장된 조회 결과를 가져오기 위해서 ModelAndView를 먼저 가져옴
					.getModelMap()			// ModelAndView에서 Model을 가져옴
						.toString());		
