...
public class EmployeeListServiceImpl implements EmployeeListService {
	...
	public void getEmployeeListUsingPagination(HttpServletRequest request, Model model) {
		...
		switch(order) {
		case "ASC" : model.addAttribute("order", "DESC"); break; // 현재 ASC정렬이므로 다음 정렬은 DESC라고 Jsp에 알려준다.
		case "DESC" : model.addAttribute("order", "ASC"); break; // 현재 DESC정렬이므로 다음 정렬은 ASC라고 Jsp에 알려준다.
		}
