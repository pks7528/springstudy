...
public class SecondServiceImpl implements ISecondService {
	...
	public ResponseEntity<BmiVO> execute1(HttpServletRequest request) {
		...
		try {
			...
			double bmi = weight / (height * height);   // bmi = 몸무게 / 키(m)*키(m), obesity 넘기기
			String obesity = null;
			if(bmi < 18.5) {
				obesity = "저체중";
			} else if(bmi < 24.9) {
			...
			BmiVO bmiVO = null;
			return new ResponseEntity<BmiVO>(bmiVO, HttpStatus.INTERNAL_SERVER_ERROR);	// HttsStatus가 500이므로 $.ajax의 error에서 처리된다.
		}
	...
	public ResponseEntity<Map<String, Object>> execute2(BmiVO bmiVO) {
		...
		if(weight == 0 || height == 0) {
			return new ResponseEntity<Map<String,Object>>(HttpStatus.BAD_REQUEST);	// 응답 코드가 정상(200)이 아니기 때문에 $.ajax의 error로 전달
		}
		
		double bmi = weight / (height * height / 10000);   // bmi = 몸무게 / 키(m)*키(m), obesity 넘기기
		String obesity = null;
		if(bmi < 18.5) {
			obesity = "저체중";
		} else if(bmi < 24.9) {
	...
	public BmiVO execute1(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			return new BmiVO(weight, height, bmi, obesity);		// $.ajax의 success로 넘기는 예외 메시지
